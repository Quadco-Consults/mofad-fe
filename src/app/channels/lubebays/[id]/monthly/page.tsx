...
export default function LubebayMonthlyPage() {
  ...
  const { data: lubebayData } = useQuery({
    queryKey: ['lubebay', lubebayId],
    queryFn: () => apiClient.get(`/lubebays/${lubebayId}/`),
  })

  const lubebay = lubebayData as Lubebay | undefined
  ...
  const handleMonthClick = (year: number, month: number) => {
    router.push(`/channels/lubebays/${lubebayId}/monthly/${year}/${month}`)
  }
        ...
        <div className="bg-white border-b">
          <div className="max-w-7xl mx-auto px-6 py-4">
            <div className="flex items-center gap-2 text-sm text-gray-600 mb-4">
              <Link href="/channels/lubebays" className="hover:text-gray-900">
                Lubebays
              </Link>
              <span>/</span>
              <Link
                href={`/channels/lubebays/${lubebayId}`}
                className="hover:text-gray-900"
              >
                {lubebay?.name || 'Loading...'}
              </Link>
              <span>/</span>
              <span className="text-gray-900 font-medium">Monthly</span>
            </div>
            <h1 className="text-2xl font-bold text-gray-900">
              {lubebay?.name || 'Lubebay'} - Monthly Performance
            </h1>
          </div>
        </div>

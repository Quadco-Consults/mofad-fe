...
export default function BinCardPage() {
  ...
  const { data: binCardData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      console.log('[BinCard FIXED] Fetching for warehouse:', warehouseId, 'product:', productId)
      const params: any = {}
      if (startDate) params.start_date = startDate
      if (endDate) params.end_date = endDate

      const result = await apiClient.getProductBinCard(warehouseId, productId, params)
      console.log('[BinCard FIXED] Success! Data:', result)
      return result
    },
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mt-6 p-4 bg-gray-100 rounded text-xs font-mono">
          <p><strong>Debug (Build v3.0):</strong></p>
          <p>Page: /warehouse/[id]/bincard/[productId]/page.tsx (OLD URL PATTERN)</p>
          <p>Warehouse ID: {warehouseId}</p>
          <p>Product ID: {productId}</p>
          <p>API Called: getProductBinCard({warehouseId}, {productId})</p>
